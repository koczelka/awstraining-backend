...
public class SNSConfig {
    ...
    AmazonSNS configureSNSClient() {
        ...
        if (snsAccessKey != null && snsSecretKey !=null) {
            //jeżeli nie korzystamy z polączenia (dla lokalnego
            return AmazonSNSClientBuilder.standard().withCredentials(new AWSStaticCredentialsProvider(new BasicAWSCredentials(snsAccessKey, snsSecretKey))).withRegion(awsRegion).build();
        } else {
